    ...
    def start(self):
        def handler(msg):
            if msg.get("message_id", None) is not None:
                if self.check_duplicate_message(
                    msg["message_id"], self.handled_message_ids
                ):
        ...
        self.kafka.listen(
            self.topics, handler, self.kafka_group_name
        )  # Handler is on_message function

    def trigger_thinking(self):
        self.thinking_module.decide(self.messages)

    def check_duplicate_message(
        self, message_id: str, handled_messages: list[str]
    ) -> bool:
