...
class MemoryModule:
    ...
    def __init__(self, collection: str = "agent_memory"):
        ...
        self.client = QdrantClient(
            host=Config().get_qdrant_host(), port=Config().get_qdrant_port()
        )
        self.encoder = SentenceTransformer(Config().get_embed_name())
        self.collection = collection
        self.embedding_dim = (
            self.encoder.get_sentence_embedding_dimension() or Config().get_embed_dim()
    ...
    def write(
        ...
        """
        Write a memory entry, if artifact ID exists, update memory entry.
        ...
        try:
            # Check exist
            exist_artifact = self.get_by_artifact_id(artifact_id, 1)
            if len(exist_artifact) == 1:
                old_artifact = exist_artifact[0]
                self.update_memory(old_artifact["memory_id"], content, metadata)
                return old_artifact["memory_id"]
    ...
    def semantic_search(
        self, query: str, top_k: int = 5, conversation_filter: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            print(
                f"[MemoryModule] Retrieved {len(memories)} memories for query: '{query[:50]}...'"
            )
            return memories
    ...
    def get_by_artifact_id(
        self, artifact_id: str, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            print(
                f"[MemoryModule] Found {len(memories)} memories for artifact: {artifact_id}"
            )
            return memories
    ...
    def get_recent_memories(
        self, limit: int = 10, conversation_filter: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def delete_memory(self, memory_id: str) -> bool:
        ...
        try:
            self.client.delete(
                collection_name=self.collection, points_selector=[memory_id]
            )
    ...
    def delete_by_artifact(self, artifact_id: str) -> int:
        ...
        try:
            ...
            if memory_ids:
                self.client.delete(
                    collection_name=self.collection, points_selector=memory_ids
                )
                print(
                    f"[MemoryModule] Deleted {len(memory_ids)} memories for artifact: {artifact_id}"
                )
    ...
    def clear_conversation(self, conversation_id: str) -> int:
        ...
        try:
            ...
            if memory_ids:
                ...
                print(
                    f"[MemoryModule] Cleared {len(memory_ids)} memories from conversation: {conversation_id}"
                )
                return len(memory_ids)
